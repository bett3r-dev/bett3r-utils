...
describe( 'Streams', function() {
  ...
  it( 'reduce over a streamed values', () => {
    const s = stream();
    const reduced = scan(( acc, curr: number ) => acc + curr, 0, s );
    s( 1 )( 2 )( 4 )( 3 );
  ...
  it( 'supports for immediate calls to dependant streams', () => {
    const s = stream();
    const hasItems = combine( function( s: flyd.Stream<number[]> ) {
      return s() !== undefined && s().length > 0;
    }, [s]);
    const immediateHasItems = immediate( combine( function( s: flyd.Stream<number[]> ) {
      return s() !== undefined && s().length > 0;
    }, [s]));
  ...
  describe( 'skip', function() {
    it( 'skip the first 3 items in a stream', ( done ) => {
      const s = stream();
      let count = 0;
      (skip( 3, s ) as flyd.Stream<number>)
        .map( num => {
          count++;
          if ( count === 1 ){
            assert.equal( num, 4 );
